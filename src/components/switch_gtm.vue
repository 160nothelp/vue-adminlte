          ...
          <div class="col-md-6">
            <el-form ref="form" :model="form" label-width="100px">
              <el-form-item label="线路查询">
                <textarea v-model="form.desc" :disabled="true" style="line-height:2.0; background-color:black;min-height:50px;height:180px;width:500px;color:rgb(45,232,29)"></textarea>
              </el-form-item>
              <el-form-item>
        ...
        <div class="row">
          ...
          <div class="col-md-6 col-md-offset-1">
            <el-button-group>
              ...
              <el-button type="primary" icon="el-icon-edit" @click="PostSwitch('default')">切换到Default默认线路</el-button>
            </el-button-group>
          </div>
          <div class="col-md-4"></div>
        </div>
    ...
    export default {
        ...
        methods:{
            PostSwitch(a){
                this.result = '';
                showLoading();
                this.$axios.post('/api/ops/switch-gtm', JSON.stringify({'type': a})).then((response) =>{
            ...
            websocketonmessage(e){ //数据接收
                ...
                let domain_line = '';
                if (redata['null']){
                    domain_line ='查询时间：' + redata['time'] + '\n查询结果：\n' +
                        '\t域名总数：'+ redata['total'] + '\n\t默认线路域名：' + redata['counter'] + '\n\t高防线路域名：'+ redata['other']
                    + '\n\t解析异常域名：' + redata['null'];
                }else {
                    domain_line ='查询时间：' + redata['time'] + '\n查询结果：\n' +
                        '\t域名总数：'+ redata['total'] + '\n\t默认线路域名：' + redata['counter'] + '\n\t高防线路域名：'+ redata['other'];
